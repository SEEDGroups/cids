...
function getContribution (){
	...
	//	debugger($sql); exit;
	 ...
	 function getTimeline(){
	     	global $conn;
		$sql = "SELECT achievement.*, event_image.image_title from achievement LEFT JOIN event_image on achievement.id = event_image.event_id WHERE achievement.event_type=2 ORDER BY achievement.id desc";
	//	debugger($sql); exit;
		$query = mysqli_query($conn, $sql);
		if(mysqli_num_rows($query) <= 0){
			return 0;
